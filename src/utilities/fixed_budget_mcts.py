#!/usr/bin/env python3
# Fixed-Budget, Fixed-Memory Root-Bandit MCTS
# Envs: Nim(15), Othello 4x4 (pass), Connect-4 5x6
# Baselines: Random, Greedy
# Harness: matchups, logging, Wilson CI, optional plots
...
# ---------------- Generic Root-Bandit MCTS ----------------

class RootBanditMCTS:
    """
    Fixed-Budget, Fixed-Memory MCTS that treats root children as a bandit:
    UCB over actions + truncated rollouts.
    O(1) per move if B,K,L are constants. Memory O(K).

    Injected env functions:
      legal_fn(state) -> list[action]       # [] means "pass" available
      step_fn(state, action) -> next_state  # action may be None (pass)
      terminal_fn(state) -> bool
      reward_fn(root_player, state) -> float in [0,1]
      state_key_fn(state) -> hashable/int   # for PRF seeds
    """
    def __init__(self, legal_fn, step_fn, terminal_fn, reward_fn, state_key_fn,
        ...
        self.L = int(L)
        self.C = float(C)
        self.m = int(stratified)  # floor samples per action
        ...
        while used < self.B:
            ...
            for a, s1 in children.items():
                if N[a] == 0:
                    ...
                    score = (W[a]/N[a]) + self.C * math.sqrt(math.log(totalN+1)/(N[a]))
                if score > best_score:
...
def run_card(env:str, matchup:str, B:int, games:int, K:int, L:int, C:float, m:int, out_dir:str):
    ...
    eff_wins = int(wins)  # conservative for draws
    p, lo, hi = wilson_ci(eff_wins, n)
    summary = {
        "env": env, "matchup": matchup, "B": B,
        "games": n, "wins": wins, "winrate": wins/n,
        ...
        "N_required": 1 + ({"nim":3,"othello4x4":8,"connect4_5x6":5}[env])  # root + children cap
    }
...
def plot_suite(env:str, summaries:pd.DataFrame, games_df:pd.DataFrame, out_dir:str):
    # 1) Win rate vs B
    ...
    for B in sorted(summaries["B"].unique()):
        sub = games_df[games_df["B"]==B]
        lat_rows.append({
            "B": B,
            "p50_ms_median": float(np.median(sub["p50_ms"])) if len(sub) else 0.0,
            "p95_ms_95pct": float(np.percentile(sub["p95_ms"],95)) if len(sub) else 0.0
        })
    df_lat = pd.DataFrame(lat_rows).sort_values("B")
    fig2 = plt.figure(figsize=(6,4))
    plt.plot(df_lat["B"], df_lat["p50_ms_median"], marker="o", label="p50 (median over games)")
    plt.plot(df_lat["B"], df_lat["p95_ms_95pct"], marker="s", label="p95 (95th percentile over games)")
    plt.xlabel("B (rollouts per move)")
    ...
    # 3) Node usage vs theoretical N
    fig3 = plt.figure(figsize=(6,4))
    plt.plot(summaries["B"], summaries["nodes_used_max"], marker="o")
    plt.plot(summaries["B"], summaries["N_required"], marker="x")
    plt.xlabel("B (rollouts per move)")
    plt.ylabel("Nodes (max used per move)")
    plt.title(f"Node usage vs theoretical N ({env})")
    plt.tight_layout()
...
def main(argv=None):
    ...
    if args.env=="nim":         K,L = 3,15
    elif args.env=="othello4x4":K,L = 8,16
    else:                       K,L = 5,30
    ...
    for B in args.B:
        for matchup in ["MCTS_vs_Random","MCTS_vs_Greedy"]:
            s, df_g, _ = run_card(args.env, matchup, B, args.games, K, L, args.C, args.stratified, args.out)
            summaries.append(s); all_games.append(df_g)
    ...
    if args.plot:
        plot_suite(args.env, df_summary, games_all, args.out)
