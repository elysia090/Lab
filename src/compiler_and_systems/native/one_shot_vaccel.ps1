<# ================================================================================
 vAccel One-Shot — v1.4.3-nf (no-finally; paste-safe, PS5.1/PS7+, ASCII only)
...
if(-not (Test-Path variable:WslIdleTimeoutMs))  { [int]$WslIdleTimeoutMs = 5000 }
...
# UInt64 helpers + RNG (xoroshiro128+)
function Convert-HexToUInt64([string]$hex) { [Convert]::ToUInt64($hex, 16) }
$U64_MAX=[UInt64]::MaxValue; $MASK32=(Convert-HexToUInt64 'FFFFFFFF'); $MASK53=(([UInt64]1 -shl 53)-1)
function Add-UInt64([uint64]$a,[uint64]$b){ $alo=$a -band $MASK32;$blo=$b -band $MASK32;$ahi=$a -shr 32;$bhi=$b -shr 32;$lo=($alo+$blo)-band $MASK32;$car=(($alo+$blo)-shr 32)-band $MASK32;$hi=(($ahi+$bhi+$car)-band $MASK32);(((($hi -band $MASK32) -shl 32) -bor ($lo -band $MASK32)) -band $U64_MAX) }
function RotateLeft-UInt64([uint64]$x,[int]$k){ $k = ($k % 64 + 64) % 64; if($k -eq 0){ return $x -band $U64_MAX }; (((($x -shl $k) -band $U64_MAX) -bor ($x -shr (64 - $k))) -band $U64_MAX) }
$GOLDEN1=(Convert-HexToUInt64 '9E3779B97F4A7C15'); $GOLDEN2=(Convert-HexToUInt64 'BF58476D1CE4E5B9')
if($Deterministic -and $Seed -gt 0){ $script:S0=[uint64]$Seed; $script:S1=([uint64]$Seed -bxor $GOLDEN1) -band $U64_MAX } else { $t=[uint64](Get-UtcNowMs); $script:S0=($t -bxor $GOLDEN1) -band $U64_MAX; $script:S1=((($t -shl 13) -band $U64_MAX) -bxor $GOLDEN2) -band $U64_MAX }
function Get-RandU64 { $s0=$script:S0;$s1=$script:S1;$res=Add-UInt64 $s0 $s1;$s1=($s1 -bxor $s0) -band $U64_MAX;$script:S0=(((RotateLeft-UInt64 $s0 55) -bxor $s1 -bxor ((($s1 -shl 14) -band $U64_MAX))) -band $U64_MAX);$script:S1=(RotateLeft-UInt64 $s1 36) -band $U64_MAX;$res -band $U64_MAX }
function Get-RandUnit01 { $r=(Get-RandU64); $mant=(($r -shr 11) -band $MASK53); [double]$mant / [double]$MASK53 }
function Get-RandInRange([double]$a,[double]$b){ $a + ((Get-RandUnit01) * ($b - $a)) }

# NDJSON writer
$Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
function Normalize-ReportPath([string]$p){ try{ [IO.Path]::GetFullPath($p) }catch{ Join-Path (Get-Location) 'scorecard.jsonl' } }
function Ensure-ReportFile([string]$Path){ $dir=Split-Path -Parent $Path; if($dir -and -not (Test-Path -LiteralPath $dir)){ New-Item -ItemType Directory -Path $dir -Force | Out-Null }; if(-not (Test-Path -LiteralPath $Path)){ New-Item -ItemType File -Path $Path -Force | Out-Null } }
function Open-ReportWriter([string]$Path){ try{ $fs=New-Object System.IO.FileStream($Path,[IO.FileMode]::Append,[IO.FileAccess]::Write,[IO.FileShare]::ReadWrite,4096,[IO.FileOptions]::SequentialScan); $sw=New-Object System.IO.StreamWriter($fs,$Utf8NoBom,4096,$false); $sw.AutoFlush=$true; $sw }catch{ Write-Warn ("Report open failed: {0}" -f $_.Exception.Message); $null } }
function Close-ReportWriter($sw){ try{ if($sw){ $sw.Flush(); $sw.Dispose() } }catch{} }
function Try-WriteNdjson($sw,[string]$Kind,[hashtable]$Fields){ if(-not $sw){ return }; try{ $rec=[ordered]@{ kind=$Kind; timeMs=(Get-UtcNowMs) }; foreach($k in $Fields.Keys){ $rec[$k]=$Fields[$k] }; $sw.Write((Convert-ToStableJson ([pscustomobject]$rec)) + [Environment]::NewLine) }catch{ Write-Warn ("NDJSON write skipped: {0}" -f $_.Exception.Message) } }
...
function Get-SystemSnapshot {
  ...
  [double]$memGB=0.0; $gpuName='unknown'; $uptimeHrs=Get-UptimeHoursSafe
  if(Get-IsWindows){
    try{
      $os=Get-CimInstance -Class Win32_OperatingSystem -ErrorAction Stop
      ...
      $osName=Get-NonNull $os.Caption $osName; $osVer=Get-NonNull $os.Version $osVer
...
function New-ExecutionPlan { $n0=[pscustomobject]@{id='n0';primitive='xread_stream';pi=1};$n1=[pscustomobject]@{id='n1';primitive='xsegment_reduce';pi=2};$n2=[pscustomobject]@{id='n2';primitive='xmatmul_tile';pi=1}; [pscustomobject]@{ planId=('p-{0}' -f (Get-UtcNowMs)); nodes=@($n0,$n1,$n2); contract=[pscustomobject]@{ barrierLayers=2; hopP95=1 } } }
function Test-Contract($Plan){ $pass=$true; foreach($n in $Plan.nodes){ if($PassBudget.ContainsKey($n.primitive)){ if($n.pi -ne $PassBudget[$n.primitive]){ $pass=$false; break } } }; $bar=$Plan.contract.barrierLayers; $q=[math]::Max(0.85,[math]::Round(1.15,2)); $hop=$Plan.contract.hopP95; [pscustomobject]@{ passOK=$pass; barrierOK=($bar -le $Config.BarrierMax); qOK=($q -le $Config.QOverQStarMax); hopOK=($hop -le 2); barrier=$bar; qOverQStar=$q; hopP95=$hop; contractOK=($pass -and ($bar -le $Config.BarrierMax) -and ($q -le $Config.QOverQStarMax) -and ($hop -le 2)) } }

# Safety
function Evaluate-Safety { $sig=Invoke-Round (Invoke-Clamp (0.996 + (((Get-RandUnit01) * 0.002) - 0.001)) 0 1) 6; [pscustomobject]@{ signatureScore=$sig; safetyOK=($sig -ge $Config.SigMin) } }

# WSL
$script:Wsl=[pscustomobject]@{ enabled=$false; started=$false; exited=$false; ok=$false; linesOut=0;linesErr=0;bytesOut=[int64]0;bytesErr=[int64]0;lastBeat=[int64]0; exitCode=$null; proc=$null }
function Test-WslAvailable { if(-not $EnableWsl -or -not (Get-IsWindows)){ return $false }; try{ $psi=New-Object System.Diagnostics.ProcessStartInfo; $psi.FileName='wsl.exe'; $psi.Arguments='--status'; $psi.UseShellExecute=$false; $psi.RedirectStandardOutput=$true; $psi.RedirectStandardError=$true; $psi.CreateNoWindow=$true; $p=New-Object System.Diagnostics.Process; $p.StartInfo=$psi; if(-not $p.Start()){ return $false }; $p.WaitForExit(); if($p.ExitCode -eq 0){ return $true }; $psi2=New-Object System.Diagnostics.ProcessStartInfo; $psi2.FileName='wsl.exe'; $psi2.Arguments='-e bash -lc "true"'; $psi2.UseShellExecute=$false; $psi2.RedirectStandardOutput=$true; $psi2.RedirectStandardError=$true; $psi2.CreateNoWindow=$true; $p2=New-Object System.Diagnostics.Process; $p2.StartInfo=$psi2; if(-not $p2.Start()){ return $false }; $p2.WaitForExit(); return ($p2.ExitCode -eq 0) }catch{ return $false } }
function Start-WslProcess([string]$Cmd){ try{ $psi=New-Object System.Diagnostics.ProcessStartInfo; $psi.FileName='wsl.exe'; $psi.Arguments='-e ' + $Cmd; $psi.UseShellExecute=$false; $psi.RedirectStandardOutput=$true; $psi.RedirectStandardError=$true; $psi.CreateNoWindow=$true; $p=New-Object System.Diagnostics.Process; $p.StartInfo=$psi; $ok=$p.Start(); if(-not $ok){ $psi.Arguments='--exec ' + $Cmd; $p=New-Object System.Diagnostics.Process; $p.StartInfo=$psi; $ok=$p.Start(); if(-not $ok){ throw 'wsl.exe failed to start' } }; $p.add_OutputDataReceived({ param($s,$e) if($null -ne $e.Data){ $script:Wsl.linesOut++; $script:Wsl.bytesOut += [Text.Encoding]::UTF8.GetByteCount($e.Data) + 1; $script:Wsl.lastBeat = Get-UtcNowMs } }); $p.add_ErrorDataReceived({ param($s,$e) if($null -ne $e.Data){ $script:Wsl.linesErr++; $script:Wsl.bytesErr += [Text.Encoding]::UTF8.GetByteCount($e.Data) + 1; $script:Wsl.lastBeat = Get-UtcNowMs } }); $p.BeginOutputReadLine(); $p.BeginErrorReadLine(); $script:Wsl.proc=$p; $script:Wsl.started=$true; $script:Wsl.ok=$true; $true }catch{ $script:Wsl.ok=$false; $false } }
function Poll-WslProcess { if(-not $script:Wsl.started){ return }; if($script:Wsl.proc -and $script:Wsl.proc.HasExited){ if(-not $script:Wsl.exited){ $script:Wsl.exitCode=$script:Wsl.proc.ExitCode; $script:Wsl.exited=$true }; return }; if(($WslIdleTimeoutMs -gt 0) -and ((Get-UtcNowMs) - $script:Wsl.lastBeat -gt $WslIdleTimeoutMs)){ try{ if($script:Wsl.proc -and -not $script:Wsl.proc.HasExited){ $script:Wsl.proc.Kill() | Out-Null } }catch{}; $script:Wsl.exitCode=-1; $script:Wsl.exited=$true } }
function Stop-WslProcess { try{ if($script:Wsl.proc){ if(-not $script:Wsl.proc.HasExited){ $script:Wsl.proc.Kill() | Out-Null }; $script:Wsl.proc.Dispose() } }catch{} }

# Observation / Score
function Invoke-Observation([pscustomobject]$Plan,$ReportWriter){
  $latP95=0.0;$frameP95=0.0;$qOver=1.0;$mpcViol=0; [int64]$ioBytes=0; [int]$missPages=0
  $power=[math]::Round((Get-RandInRange 18 28),1); $temp=[math]::Round((Get-RandInRange 45 62),1)
  $compRatio=0.12; $deltaHit=0.25
  $timer=[System.Diagnostics.Stopwatch]::StartNew(); [int]$tick=0
  while($timer.Elapsed.TotalSeconds -lt $DurationSec){
    Start-Sleep -Milliseconds $Config.TickMs; $tick++
    $ioBytes += [int64]([math]::Round((Get-RandInRange 30000000 80000000),0))
    $missPages += [int]([math]::Round((Get-RandInRange 100 600),0))
    $latP95 = (Get-RandInRange ($LatencyBudgetMs*0.7) ($LatencyBudgetMs*1.05)); if($latP95 -lt 8){ $latP95=8 }; $latP95=[math]::Round($latP95,1)
    ...
    if(($tick % $LogEveryTicks) -eq 0){
      $kv=@{ runId=$Plan.planId; latAvgMs=[math]::Round(($latP95*0.8),1); latP95Ms=$latP95; ioBytes=[int64]$ioBytes; missPages=$missPages; powerW=$power; tempC=$temp; qOverQStar=$qOver }
      if($EnableWsl -and $script:Wsl.enabled){ $kv.wslLinesOut=$script:Wsl.linesOut; $kv.wslLinesErr=$script:Wsl.linesErr; $kv.wslBytesOut=[int64]$script:Wsl.bytesOut; $kv.wslBytesErr=[int64]$script:Wsl.bytesErr; $kv.wslLastBeat=$script:Wsl.lastBeat; $kv.wslExited=$script:Wsl.exited }
...
function Get-ExitCode($Score){ $ok=$Score.contractOK -and $Score.safetyOK -and $Score.sloOK -and $Score.reuseOK; if($Gate -eq 'memcomp'){ $ok=$ok -and $Score.memOK }; if($ok){ 0 } elseif($Gate -eq 'strict' -or $Gate -eq 'memcomp'){ 90 } else { 10 } }
...
function Invoke-AllTests {
  try{
    ...
    if(T 'Determinism'){ $localS0=$script:S0; $localS1=$script:S1; $script:S0=[uint64]123; $script:S1=([uint64]123 -bxor $GOLDEN1); $a1=@(); 1..6 | % { $a1 += ,(Get-RandU64) }; $script:S0=[uint64]123; $script:S1=([uint64]123 -bxor $GOLDEN1); $a2=@(); 1..6 | % { $a2 += ,(Get-RandU64) }; Assert-True (@(Compare-Object $a1 $a2).Count -eq 0) 'Determinism' "sequence mismatch"; $script:S0=$localS0; $script:S1=$localS1 }
    if(T 'Snapshot-no-nulls'){ $sys=Get-SystemSnapshot; $ok = ($sys.OsName) -and ($sys.OsVersion -ne $null) -and ($sys.CpuName) -and ($sys.LogicalCPU -ge 1); Assert-True $ok 'Snapshot-no-nulls' "missing fields" }
    if(T 'Report-ndjson'){ $tmp=Join-Path $env:TEMP ("vaccel-{0}.jsonl" -f (Get-UtcNowMs)); Ensure-ReportFile $tmp; $w=Open-ReportWriter $tmp; Try-WriteNdjson $w 'metric' @{ foo=1; bar='x' }; Close-ReportWriter $w; $line=Get-Content -LiteralPath $tmp -TotalCount 1; $obj=$line | ConvertFrom-Json; Assert-True ($obj.kind -eq 'metric' -and $obj.foo -eq 1 -and $obj.bar -eq 'x') 'Report-ndjson' "json roundtrip"; Remove-Item -LiteralPath $tmp -Force -ErrorAction SilentlyContinue }
    if(T 'WSL-available-bool'){ $ok = Test-WslAvailable; Assert-True ($ok -is [bool]) 'WSL-available-bool' "availability should return bool" }
    return $true
  }catch{
    Write-Err ("Test: " + $_.Exception.Message)
...
if($RunTests){ $ok = Invoke-AllTests; & $Cleanup; return }
...
} else { Write-Info "WSL not available or disabled" }
...
$script:ExitCode  = Get-ExitCode $score
Try-WriteNdjson $writer 'summary' @{ runId=$plan.planId; exitCode=$script:ExitCode; scorecard=$score }

switch($Output){
  'json'   { ([pscustomobject]@{ runId=$plan.planId; exitCode=$script:ExitCode; scorecard=$score } | Convert-ToStableJson) | Write-Output }
  'ndjson' { ([pscustomobject]@{ runId=$plan.planId; exitCode=$script:ExitCode; scorecard=$score } | Convert-ToStableJson) | Write-Output }
  'plain'  { "{0} {1}" -f $plan.planId,$script:ExitCode | Write-Output }
  'tsv'    { "runId`texitCode"; "{0}`t{1}" -f $plan.planId,$script:ExitCode | Write-Output }
  default  {
    Write-Host ""
    Write-Host ("vAccel One-Shot  runId={0}" -f $plan.planId)
    Write-Host ("ExitCode={0}  Gate={1}" -f $script:ExitCode,$Gate)
    ...
    } else { Write-Host "WSL: disabled or unavailable" }
    if($ExitHints -and $script:ExitCode -ne 0 -and $score.reasons -and $score.reasons.Length -gt 0){
