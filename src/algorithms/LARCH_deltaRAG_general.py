...
class Config:
    ...
    audit_max:int=0
    grid_exact:bool=False     # use 4-phase lattice cover
    grid_oracle:bool=False    # exact O(1) on uniform unit grid
...
class CoverBuilder:
    ...
    def _grid_phase_centers(self,R:int)->List[int]:
        ...
        centers=set()
        step=max(2*R,2)  # FIX: spacing is 2R
        for ox in (0,R):
            for oy in (0,R):
                x=ox
                while x<W:
    ...
    def build(self)->List[CoverScale]:
        ...
        for cs in cover:
            ...
            if self.cfg.grid_exact and self.g.coords is not None and float(cs.R).is_integer():
                W=self.g.grid_w; H=self.g.grid_h
                def vid(x,y): return y*W+x
                step=max(2*int(cs.R),2)  # FIX: adjacency step is 2R
                    ...
                    for dx,dy in ((step,0),(-step,0),(0,step),(0,-step)):
                        ...
                        if 0<=nx<W and 0<=ny<H:
                            ...
                            if nb in centers:
                                ...
                                # For axis neighbor at distance 2R, intersection is a single point (midpoint)
                                mx, my = (x0+nx)//2, (y0+ny)//2
                                p = vid(mx,my)
                                if p in inter:
                                    cs.portals[frozenset((c,nb))]=[p]
                                else:
                                    # robust fallback: choose argmin da+db within inter
                                    best=None; bestp=None
                                    for x in inter:
                                        da=A.dist_to_center.get(x,math.inf)
                                        db=B.dist_to_center.get(x,math.inf)
                                        s=da+db
                                        if best is None or s<best: best=s; bestp=x
    ...
    def build_sets(self,cover:List[CoverScale])->Dict[int,List[int]]:
        ...
        for cs in cover:
            ...
            for v in range(self.g.n):
                ...
                # Also add owner centers from coarser to finer (chain) as constants
                # (here, centers themselves serve as hubs; constant per scale)
...
class QueryEngine:
    ...
    def query(self,u:int,v:int)->Tuple[float,int,bool,bool,bool,bool]:
        ...
        # Grid oracle (exact O(1)) if enabled and applicable
...
class Evaluator:
    def __init__(self,cfg:Config,g:Graph):
        ...
        self.max_hubs=max(len(s) for s in H_sets.values()) if H_sets else 0
