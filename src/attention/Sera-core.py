...
# -*- coding: utf-8 -*-
"""
Sera-core.py (v2.1 — integrated predictable gating + overlay manager)
- Stable SOS memory (low-pass biquad cascade, DC gain = 1/section)
- PRF attention with query-only whitening, clipping, denominator floor
- CCR truncated corrector
- GateClamp:
  * original threshold/horizon policy
  * predictable smooth cap policy (depends only on t-1 logs; monotone)
- OverlayManager:
  * Type-A/B: fixed small K anchors with scalar coeffs (d_v=1 demo)
  * Type-C hook present (requires d_v>1 & r_v>0) but not used in default demo
All hot-path loops are over configuration-time constants.
"""
...
def safe_sigmoid(u: float) -> float:
    if u >= 0.0:
        z = math.exp(-u); return 1.0 / (1.0 + z)
    else:
        z = math.exp(u);  return z / (1.0 + z)
...
class PRFAttn:
    def __init__(
        self, d: int, d_v: int = 1, r: int = 256, tau: float = 1.3, gamma: float = 0.985,
    ...
    def _phi(self, x: np.ndarray) -> np.ndarray:
        ...
        if self.clip_c is not None:
            pc = np.clip(proj, -self.clip_c, self.clip_c); self.clip_hits += int(np.sum(pc!=proj)); proj=pc
...
class CCRCorrector:
    def __init__(self, V: int=3, rho: float=0.2, m: int=3) -> None:
        ...
        B=np.zeros((E,self.V))
        for e in range(E):
            i=e; j=(e+1)%self.V; B[e,i]=+1.0; B[e,j]=-1.0
...
class GateClamp:
    ...
    def policy_update_threshold(self, RJ: float, margin: float) -> None:
    ...
    def policy_apply_smooth(self, RJ_prev: float, margin_prev: float, cap_hi: float=0.9, alpha: float=1.4, beta: float=0.3, RJ0: float=0.25) -> None:
        term_RJ = math.exp(-alpha * max(0.0, float(RJ_prev) - RJ0))
        term_m  = (1.0 - beta * max(0.0, -float(margin_prev)))
        cap_raw = cap_hi * term_RJ * term_m
        self.cap = min(1.0, max(self.cap_low, cap_raw))
...
class SeraConfig:
    ...
    gate_policy: str="threshold"     # "threshold" or "smooth"
    cap_hi: float=0.9; alpha: float=1.4; beta: float=0.3; RJ0: float=0.25

class OverlayManager:
    def __init__(self, K: int=0):
        self.K=int(K); self.anchors: List[np.ndarray]=[]
        self.u: Optional[np.ndarray]=None; self.beta: Optional[np.ndarray]=None
        self.typeC_cores: List[Dict]=[]   # for d_v>1
        self.enabled=False
    def set_anchors(self, anchors: List[np.ndarray]):
        self.anchors = [np.asarray(a, dtype=float) for a in anchors]
        self.K=len(self.anchors)
    def set_typeA_B(self, u: np.ndarray, beta: np.ndarray):
        self.u=np.asarray(u,dtype=float); self.beta=np.asarray(beta,dtype=float)
    def enable(self, flag: bool=True): self.enabled=bool(flag)
    def build_overlays(self)->Optional[Dict[str,List]]:
        if not self.enabled or self.K==0: return None
        ov={"A":[],"B":[]}
        for i in range(self.K):
            ov["A"].append((self.anchors[i], np.array([float(self.u[i])])))
            ov["B"].append((self.anchors[i], float(max(0.0, self.beta[i]))))
        return ov
...
class SeraCore:
    def __init__(self, cfg: SeraConfig) -> None:
        ...
        self.last_RJ=0.0; self.last_margin=0.0
        self.ovl=OverlayManager(K=0)

    def step_ingest(self, k: np.ndarray, v: np.ndarray) -> None:
        self.attn.ingest(k,v)

    def fuse_predict(self, q: np.ndarray, RJ: float=0.0, margin: float=0.0):
        # predictable smooth cap (depends only on last logs)
        if self.cfg.gate_policy=="smooth":
            self.gate.policy_apply_smooth(self.last_RJ, self.last_margin, self.cfg.cap_hi, self.cfg.alpha, self.cfg.beta, self.cfg.RJ0)
        # attention (with overlays if enabled)
        overlays = self.ovl.build_overlays()
        y_vec, info = self.attn.query(q, overlays=overlays)
        y_att=float(np.sum(y_vec))
        # CCR
        y_loc=np.array([y_att,y_att,y_att]); y_star,_=self.ccr.correct(y_loc)
        y_ccr=float(np.mean(y_star))
        # memory
        y_lin=float(self.mem.step(y_ccr))
        # threshold policy (uses current diagnostics)
        if self.cfg.gate_policy=="threshold":
            self.gate.policy_update_threshold(RJ, margin)
        g,_=self.gate.forward(RJ, margin, info["den"])
        y = g*y_ccr + (1.0-g)*y_lin
        # update last diagnostics AFTER prediction
        self.last_RJ=float(RJ); self.last_margin=float(margin)
        return {"y":y, "y_att":y_att, "y_ccr":y_ccr, "y_lin":y_lin, "g":g, "den":float(info["den"])}

    def snapshot(self) -> Dict[str,dict]:
        return {"attn":{"W":self.attn.W.copy(),"R":self.attn.R.copy(),"s":self.attn.s.copy(),"mu":self.attn.mu.copy(),"sig2":self.attn.sig2.copy(),"cR":self.attn.cR.copy(),"cs":self.attn.cs.copy()},
                "mem":{"z1":self.mem.z1.copy(),"z2":self.mem.z2.copy(),"b0":self.mem.b0.copy(),"b1":self.mem.b1.copy(),"b2":self.mem.b2.copy(),"a1":self.mem.a1.copy(),"a2":self.mem.a2.copy()},
                "prev_hash":self.prev_hash}

    def restore(self, snap: Dict[str,dict]) -> None:
        a=snap["attn"]; self.attn.W=a["W"].copy(); self.attn.R=a["R"].copy(); self.attn.s=a["s"].copy()
        self.attn.mu=a["mu"].copy(); self.attn.sig2=a["sig2"].copy(); self.attn.cR=a["cR"].copy(); self.attn.cs=a["cs"].copy()
        m=snap["mem"]; self.mem.z1=m["z1"].copy(); self.mem.z2=m["z2"].copy(); self.mem.b0=m["b0"].copy(); self.mem.b1=m["b1"].copy(); self.mem.b2=m["b2"].copy(); self.mem.a1=m["a1"].copy(); self.mem.a2=m["a2"].copy()
