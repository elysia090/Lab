...
# AbsorptionProjection: 吸収合成によりクエリをキー空間へ写像 (出力: [B, L, d_key])
...
# FastAttention 本体 (出力: [B, L, d_model])
class FastAttention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
        ...
        for head_idx in range(self.config.n_heads):
            ...
            # Advanced indexing で候補キー抽出:
            # K_proj: [B, L, d_key] → candidate_keys: [B, L, k_max, d_key]
            ...
            # 値抽出: 同様に advanced indexing を用いて key_down から取得 → 候補値: [B, L, k_max, d_key]
