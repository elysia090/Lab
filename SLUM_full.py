...
class SLUM:
    def __init__(self, cfg: SLUMConfig):
        ...
        self.s = KahanVec(cfg.r, dtype=np.float64)
        self._buf_V = []; self._steps = 0
    ...
    def update(self, k_t, v_t):
        phi_t = self.feat.phi(k_t)
        r_t = self._project_coeff(v_t)
        if self.cfg.gamma != 1.0:
            self.A.scale(self.cfg.gamma); self.s.scale(self.cfg.gamma)
        self.A.add(np.outer(phi_t, r_t))
        self.s.add(phi_t)
        self._buf_V.append(v_t.astype(np.float64))
        if len(self._buf_V) > self.cfg.basis_update_B: self._buf_V.pop(0)
        self._steps += 1
    ...
    def _maybe_update_basis(self):
        if len(self._buf_V) < self.cfg.r_v: return
        V = np.stack(self._buf_V, axis=1)
        try:
        ...
        Q = Uq @ Vq
        self.A.sum = self.A.sum @ Q; self.A.c = self.A.c @ Q
        self.U = self.U @ Q
        self._update_ridge_matrix()
    def query(self, q):
        phi_q = self.feat.phi(q)
        num_small = phi_q @ self.A.value()
...
def experiment_latency_vs_n(outdir, d=64, d_v=128, tau=None, r=256, r_v=16, n_list=(512,1024,2048,4096,8192), gamma=1.0, lam=1e-3, mu=1e-3, seed=0):
    ...
    for n in n_list:
        ...
        cfg = SLUMConfig(d=d, d_v=d_v, r=r, r_v=r_v, tau=tau, gamma=gamma, lam=lam, mu=mu, basis_update_B=200, seed=seed)
        model = SLUM(cfg)
        t0 = time.time(); model.ingest_sequence(K, V)
        q = rng.randn(d).astype(np.float64); q /= (np.linalg.norm(q)+1e-12)
        _ = model.query(q); t1 = time.time()
        slum_ms = (t1 - t0)*1000.0
        t0 = time.time(); _ = softmax_attention_exact(q, K, V, tau); t1 = time.time()
        exact_ms = (t1 - t0)*1000.0
        rows.append({"n": n, "slum_ms": slum_ms, "exact_ms": exact_ms})
    csv_path = os.path.join(outdir, "slum_latency_vs_n.csv")
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["n","slum_ms","exact_ms"]); writer.writeheader()
        for row in rows: writer.writerow(row)
    xs = np.array([row["n"] for row in rows], dtype=np.float64)
    ys1 = np.array([row["slum_ms"] for row in rows], dtype=np.float64)
    ys2 = np.array([row["exact_ms"] for row in rows], dtype=np.float64)
    plt.figure(); plt.plot(xs, ys1, marker="o", label="SLUM ingest+query"); plt.plot(xs, ys2, marker="o", label="Exact softmax query")
    plt.xlabel("n (sequence length)"); plt.ylabel("time (ms)"); plt.title("SLUM latency vs n"); plt.legend()
    png_path = os.path.join(outdir, "slum_latency_vs_n.png"); plt.savefig(png_path, bbox_inches="tight"); plt.close()
    return csv_path, png_path

def experiment_check(outdir, d=32, d_v=64, tau=None, r=64, r_v=8, n=256, gamma=1.0, lam=1e-3, mu=1e-3, seed=1):
    os.makedirs(outdir, exist_ok=True)
    rng = np.random.RandomState(seed)
    if tau is None: tau = math.sqrt(d)
    ...
    cfg = SLUMConfig(d=d, d_v=d_v, r=r, r_v=r_v, tau=tau, gamma=gamma, lam=lam, mu=mu, basis_update_B=50, seed=seed)
    model = SLUM(cfg); model.ingest_sequence(K, V)
    q = rng.randn(d).astype(np.float64); q /= (np.linalg.norm(q)+1e-12)
    y_hat = model.query(q)
    const_err = LA.norm(y_hat - v_const) / (LA.norm(v_const)+1e-12)
    for t in range(100):
        k = rng.randn(d); k /= (LA.norm(k)+1e-12); v = rng.randn(d_v); model.update(k, v)
    y2 = model.query(q)
    drift = LA.norm(y2 - y_hat) / (LA.norm(y_hat)+1e-12)
    csv_path = os.path.join(outdir, "slum_checks.csv")
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["const_err","basis_drift"]); writer.writeheader()
        writer.writerow({"const_err": float(const_err), "basis_drift": float(drift)})
    return csv_path
...
def main():
    ...
    if args.cmd == "err_r":
        ...
        c4 = experiment_check(outdir=outdir)
        print(c1); print(p1); print(c2); print(p2); print(c3); print(p3); print(c4)
    else:
        parser.print_help()
