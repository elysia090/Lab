Hlm x Sera x Enc x Gen x Cluster x Multimodal
...
Optional meta-linear head (Hlm) on [y_hat_self, {y_hat_m}] for 2 - (K+1) dims.
...
[Clients] -> (Ingress Gate) -> [ShardRouter]
  -> S_h:   Gen(txt self), Hlm(gate/linear), Sera_txt(dec)
  -> S_img: Enc_img + Sera_img(enc)
  -> S_aud: Enc_aud + Sera_aud(enc)
  -> S_vid: Enc_vid + Sera_vid(enc)

Gen step:
S_h pulls at most K_mod {y_hat_m, den_m, RJ_m, margin_m, ver_sera_m} from remote shards using cached PRF basis + pinned versions;
...
if Hlm pressure (ring occupancy up or emergency used) at S_h:
...
6. COMPLEXITY AND BOUNDS
   Per token on S_h (decoder):
    - 1 x Sera_txt self + up to K_mod cross Sera_m reads (RPC overlap)
    - 1 x Hlm gate + 1 x Hlm linear add-on
...
7. MULTIMODAL ENCODER DETAILS (PER MODALITY)
   7.1 Audio specifics
    ...
    - Sera_aud gamma_aud chosen from desired acoustic memory (e.g., 1 - 5s -> gamma ~ exp( - hop/tau_mem)).
...
Concatenate stabilized readouts y_hat_m and diagnostics into an Hlm feature vector; Hlm does final linear regression -> maximally interpretable.
...
pin_versions({ver_sera_txt_dec, ver_base, ver_delta}, {ver_sera_m_enc for m in catalog})
...
12. FAILURE MODES AND PLAYBOOKS
    ...
    Action: use cached y_hat_m <= B steps; if exceeded, drop m from M_used; clamp G_m -> 0; bias toward self/text or Hlm linear; log predictable actions.

12.2 Cluster-wide margin dip
Action: RiskOrchestrator issues global clamp and temperature cap; beta_floor and lambda_* raised predictably; Hlm lr scaled down; announce horizon T if gamma ~ 1.
...
14. DEFAULTS (GOOD FIRST RUN)
     ...
     - Hlm: d=2,b=4,S=8,L=8,Q=16; tau_low=0.6, tau_high=0.8; C s.t. slack >= 2 x (p99 lambda_max T_rb).
...
15. END-TO-END SLOS
     ...
     - Sera error: O(r_m^{-1/2}) uniform in time for gamma_m<1; margins anytime-valid.
...
17. CHECKLIST (BRING-UP)
    [ ] Per-modality Enc_m + Sera_m online (gamma_m, r_m sized; clipping logged).
