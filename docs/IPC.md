...
1.1.2 A slot encoding that carries variable-length payloads up to payload_capacity within each slot.
...
2.1.3 The implementation MUST use FUTEX_WAIT with expected-value semantics and FUTEX_WAKE for wakeups.
2.2 Architecture
...
2.3.2 Implementations MAY tune spinning and batching, but MUST NOT change ABI layout.
	3.	Primitive model
3.1 Data plane
...
5.1.3 total_size MUST equal header_size + ring_bytes in v0.0.1.
5.2 Mapping
...
6.3.3 ring_offset MUST be aligned to 64.
...
7.1 The header is a fixed binary layout. Implementations MUST NOT rely on compiler struct layout; they MUST treat the mapping as bytes and use fixed offsets below.
7.2 All reserved fields MUST be zero on create and MUST be validated as zero on attach.
7.3 Header offsets (from mapping base)
7.3.1 0x000 magic: u64
7.3.2 0x008 version_major: u16
7.3.3 0x00A version_minor: u16
7.3.4 0x00C header_size: u32 (MUST be 0x180)
7.3.5 0x010 total_size: u64
7.3.6 0x018 ring_offset: u64
7.3.7 0x020 ring_bytes: u64
7.3.8 0x028 arena_offset: u64 (MUST be 0 in v0.0.1)
7.3.9 0x030 arena_bytes: u64 (MUST be 0 in v0.0.1)
7.3.10 0x038 capacity_pow2: u8
7.3.11 0x039 reserved0: [u8; 7] (MUST be 0)
7.3.12 0x040 slot_size: u32
7.3.13 0x044 reserved1: u32 (MUST be 0)
7.3.14 0x048 flags: AtomicU32
7.3.15 0x04C reserved2: u32 (MUST be 0)
7.3.16 0x050 producer_pid: AtomicU32 (diagnostic only)
7.3.17 0x054 consumer_pid: AtomicU32 (diagnostic only)
7.3.18 0x058 error_code: AtomicU32 (diagnostic only)
7.3.19 0x05C reserved3: u32 (MUST be 0)
7.3.20 0x060 reserved4: [u8; 32] (MUST be 0)
7.3.21 0x080 head: AtomicU64
7.3.22 0x0C0 tail: AtomicU64
7.3.23 0x100 doorbell_ne: AtomicI32
7.3.24 0x140 doorbell_nf: AtomicI32
...
9.6 Wrap behavior
9.6.1 Wrap is permitted. Correctness relies only on the invariant that producer never advances head more than cap ahead of tail.
9.6.2 Implementations MUST use wrapping_add for local head/tail increments to preserve modulo-2^64 semantics.
...
10.5 Initialization publish
10.5.1 Creator MUST initialize all header fields, ring bytes, head=0, tail=0, doorbells to 0.
10.5.2 Creator MUST set flags.INITIALIZED using a Release operation after initialization is complete.
10.5.3 Attach MUST read flags with Acquire and MUST NOT operate until INITIALIZED is observed.
...
12.4.2.4 If flags.SHUTDOWN set, return Shutdown.
12.4.2.5 If flags.PRODUCER_CLOSED set and h == t, return Closed.
12.4.2.6 Spin phase: for i in 0..spin_iters
12.4.2.6.1 cpu_relax
12.4.2.6.2 recheck h=head.load(Acquire) and t as above, break if h!=t
12.4.2.7 epoch = doorbell_ne.load(Relaxed).
12.4.2.8 Recheck empty using fresh h=head.load(Acquire) and t.
12.4.2.9 If still empty, futex_wait(doorbell_ne, epoch, timeout).
12.4.2.10 If timed out, return Timeout.
12.4.2.11 Loop.
...
12.5.2.4.2 futex_wake(doorbell_ne, 1)
12.6 Not-full wait (producer, optional)
12.6.1 Enabled iff flags.NOT_FULL_ENABLED is set at init and observed by producer.
...
12.6.3.5 Spin phase: for i in 0..spin_iters recheck full (refresh t with Acquire permitted).
...
12.7.2.3 Let was_full = (h.wrapping_sub(t) == cap).
12.7.2.4 Publish tail.store(Release, t+1 modulo 2^64).
12.7.2.5 If was_full then
12.7.2.5.1 doorbell_nf.fetch_add(1, Relaxed)
12.7.2.5.2 futex_wake(doorbell_nf, 1)
12.8 Close wake
12.8.1 close_producer MUST wake doorbell_ne with wake-all.
12.8.2 close_consumer MUST wake doorbell_nf with wake-all.
12.9 Shutdown wake
...
13.4.2.8 If NOT_FULL_ENABLED is set, compute was_full using fresh h_now = head.load(Acquire) and current tail_local:
13.4.2.8.1 was_full = (h_now.wrapping_sub(tail_local) == cap)
...
13.5.1 Batch operations MAY be provided. They MUST NOT introduce CAS/RMW on head/tail and MUST NOT call futex in hot paths where progress is possible.
13.5.2 push_batch(k items)
13.5.2.1 Determine was_empty exactly once before publishing:
13.5.2.1.1 Read t0 = tail.load(Acquire).
13.5.2.1.2 was_empty = (head_local == t0).
13.5.2.2 Write k slots (payload+header) sequentially.
13.5.2.3 Publish head exactly once with Release: head.store(Release, head_local + k modulo 2^64).
13.5.2.4 If was_empty and k>0, perform exactly one not-empty wake (doorbell_ne fetch_add + futex_wake(1)).
13.5.3 pop_batch(up to k items)
13.5.3.1 If NOT_FULL_ENABLED, compute was_full exactly once before publishing tail:
13.5.3.1.1 Read h0 = head.load(Acquire).
13.5.3.1.2 was_full = (h0.wrapping_sub(tail_local) == cap).
13.5.3.2 Consume up to k available items, copying out each payload.
13.5.3.3 Publish tail exactly once with Release: tail.store(Release, tail_local + n modulo 2^64), where n is the number popped.
13.5.3.4 If was_full and n>0, perform exactly one not-full wake (doorbell_nf fetch_add + futex_wake(1)).
13.5.4 Batch operations MUST preserve the same visibility guarantees as single push/pop: slot writes happen-before head publish (Release), and head observe (Acquire) happens-before slot reads.
	14.	Close semantics
14.1 close_producer()
14.1.1 Sets flags.PRODUCER_CLOSED via fetch_or(Release or AcqRel).
14.1.2 Wakes doorbell_ne with futex_wake(doorbell_ne, INT_MAX).
14.1.3 After close_producer, producer MUST NOT push.
14.2 close_consumer()
14.2.1 Sets flags.CONSUMER_CLOSED via fetch_or(Release or AcqRel).
14.2.2 Wakes doorbell_nf with futex_wake(doorbell_nf, INT_MAX).
...
15.1.11 arena_offset == 0 and arena_bytes == 0 (v0.0.1 reserves these and forbids nonzero).
...
16.2 Operations
16.2.1 create(config) -> fd, total_size (initializes header/ring and sets INITIALIZED)
16.2.2 attach(fd, total_size) -> ShmQueue (validates)
16.2.3 into_producer/into_consumer enforce cross-process uniqueness via flags compare_exchange
...
17.16 Syscall { op: SysOp, errno: i32 }
...
20.1 Implements the header ABI exactly (fixed header_size and offsets) and validates on attach.
...
20.6 For NOT_FULL_ENABLED, was_full detection MUST use a fresh head.load(Acquire) (no cached-head shortcuts).
