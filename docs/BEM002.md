...
BEM v0.0.1 defines a hardware-oriented, finite-state abstract machine with:

– bit-sliced global state s_t and shared memory M_t,
– a structured observation pipeline producing task-aware, context-aware features,
– an ANN-plus-bandit routing scheme that selects experts with bounded O(1)-like cost per step,
– Boolean experts (ANF/ROBDD) evaluated in bit-sliced form across W lanes,
– log-domain multiplicative weights with adaptive, per-task learning rates and bandit statistics,
– structural updates (split, merge, macro extraction, superoptimization) driven by trace analysis and error localization,
– a verification kernel with incremental CNF, Hoare annotations, and CEGIS-style synthesis and counterexample handling,
– optional PoX objectives and scheduling that direct long-running computation toward regret reduction, safety, cost reduction, and information gain.

All core semantics are defined using integers, bits, table lookups, and bounded-depth search. Neural or floating-point models are treated as external, untrusted proposers whose outputs MUST pass through the verification kernel before they can affect the machine’s trusted fast path.
