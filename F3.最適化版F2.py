...
メモ：
compute_predecessors の並列化:
– 辞書形式からエッジリストに変換し、NumPy のベクトル演算（np.argsort と np.searchsorted）で前駆リスト（＝sorted_src）と各頂点の開始オフセットを一括計算する。
– これにより逐次ループを回避し、高速な C レベル処理を利用可能。

prepare_graph_arrays の並列化:
– 各頂点ごとの前駆数を np.bincount で一括計算し、その結果に対して排他的スキャン（prefix-sum）を NumPy で計算する。
– 各頂点の前駆リストのコピー処理は、Numba の prange などを用いた並列ループにより高速化できる（または、上記のベクトル化手法で完全に置き換える）。
