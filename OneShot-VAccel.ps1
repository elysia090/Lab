& {
  # ============================================================================
  # vAccel One-Shot — v1.3.6b (inline, ASCII-only, PS 5.1+/7, O(1)/tick, WSL)
  # Usage example:
  #   paste & run as-is, or tweak the parameters below before pressing Enter.
  # ============================================================================

  # ----------------------- Parameters (edit here) -----------------------------
  [string] $ProfilesCsv        = 'AUTOHOOK-GLOBAL,LIMIT-ORDER,SUPRA-HIEND,RT-BALANCED,FABRIC,MEMZERO'
  [string] $Aggressiveness     = 'balanced'   # conservative|balanced|aggressive
  [int]    $LatencyBudgetMs    = 60
  [double] $PowerBudgetW       = 45.0
  [double] $TempBudgetC        = 85.0
  [int]    $DurationSec        = 10
  [bool]   $Explain            = $true        # prints system/plan/observed summaries
  [string] $Gate               = 'strict'     # none|strict|memcomp
  [string] $OutputMode         = 'human'      # human|json|ndjson|plain|tsv
  [int]    $LogEveryTicks      = 2            # write metrics every N ticks (>=1)

  # Determinism
  [bool]   $Deterministic      = $true
  [uint64] $Seed               = 42           # used when Deterministic=$true
  ...
  if ($LogEveryTicks -lt 1) { $LogEveryTicks = 1 }
  # ---------------------------------------------------------------------------
  ...
  function Open-Report([string]$Path){
    $fs = New-Object System.IO.FileStream($Path, [System.IO.FileMode]::Append, [System.IO.FileAccess]::Write, [System.IO.FileShare]::ReadWrite, 4096, [System.IO.FileOptions]::SequentialScan)
    $sw = New-Object System.IO.StreamWriter($fs, $Utf8NoBom, 4096, $true)
    $sw.AutoFlush = $true
    $sw
  }
  function Close-Report($sw){
    try { if ($sw) { $sw.Flush(); $sw.Dispose() } } catch {}
  }
  ...
  # minimal constant-size plan to keep hot path O(1)
  $PassBudget = @{
  ...
  function Start-WslJob([string]$Cmd){
    try {
      ...
      $psi.Arguments = '-e ' + $Cmd   # pass "bash -lc '...'"
  ...
  function Build-Score([pscustomobject]$C,[pscustomobject]$S,[pscustomobject]$O){
  ...
  function Get-ExitCode([pscustomobject]$S){
