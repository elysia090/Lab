...
class VecAddKernel:
    ...
    def generate_llvm_ir(self) -> str:
        """
        グリッドストライドループを用いて各スレッドが複数要素を処理する LLVM IR を生成。
        NVVM 用メタデータと llvm.used も付与します。
        """
        ...
        # NVVM 用メタデータの付与（定数式として "i8* bitcast(...)" で記述）
        llvm_ir = str(module) + """
!nvvm.annotations = !{!0}
...
"""
    ...
    def optimize_llvm_ir(self, llvm_ir: str) -> str:
        """
        llvmlite.binding により LLVM IR の最適化パスを実行し、最適化済み IR を返す。
        """
        llvm.initialize()
    ...
    def compile_to_ptx(self, optimized_ir: str) -> str:
        """
        最適化済み LLVM IR から llc を呼び出し PTX コードを生成、文字列として返す。
        """
    ...
    def run(self, a_np: np.ndarray, b_np: np.ndarray) -> np.ndarray:
        """
        入力配列 a, b に対してカーネルを実行し、結果配列 c を返す。
        (ホスト側の処理全体の時間を含む)
        """
        N = a_np.size
...
def benchmark_kernel_host(kernel: VecAddKernel, a_np: np.ndarray, b_np: np.ndarray, iterations: int = 10):
    """
    ホスト側で kernel.run() を複数回実行し、平均実行時間を測定する。
    (データ転送も含む)
    """
    times = []
...
def benchmark_kernel_gpu(kernel: VecAddKernel, a_np: np.ndarray, b_np: np.ndarray, iterations: int = 10):
    """
    CUDA のイベントを用いて、GPU 上のカーネル実行時間のみを測定する。
    (データ転送は含まれない)
    """
    N = a_np.size
    block_size = 256
    ...
    for _ in range(iterations):
        ...
        # 修正: end_event.time_since(start_event) を用いて GPU 側の実行時間を取得
...
def main():
    ...
    # カーネル生成（PTX はキャッシュされるので複数回呼び出してもオーバーヘッドは発生しない）
