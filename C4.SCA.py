#State space estimation using an extended karma filter  by the Clifford wide-area linear estimation method incorporating the ARIMA model
...
def initialize_models(train_data, obs_data):
    arima_model = ARIMA(train_data['sales'].values, order=(1, 1, 1))
    arima_results = arima_model.fit()
    ar_params = arima_results.arparams
    ma_params = arima_results.maparams
    state_space_model = StateSpaceModel(ar_params, ma_params)
    initial_state = np.zeros(len(ar_params) + len(ma_params))
